...
class OkxExchange(BaseExchange, ABC):
    ...
    async def open_spot_market_orders(self, symbols: list[str], amount_usdt: float):
        try:
            ...
            for symbol in symbols:
                ...
                if markets[symbol]['info']['state'] != 'live':
                    unsupported_symbols.append(symbol)
                    continue

                if markets[symbol]['info']['instType'] != 'SPOT':
                    unsupported_symbols.append(symbol)
                    continue
            ...
            for symbol in symbols:
                ...
                order = await self.exchange.create_market_buy_order(
                    symbol=symbol, amount=order_amount, params=params
                )
                orders.append(order)
